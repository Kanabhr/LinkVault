...
const SaveLinks = AsyncHandler(async (req, res) => {
  ...
  const populatedSave = await urldata.findById(savedata._id).populate("customTagId");
...
const getPublicBookmarks = AsyncHandler(async (req, res) => {
  ...
  const Publicbookmarks = await urldata.find({ userId: userbyname._id }).populate("customTagId").populate("userId", "username useremail").skip(offset).limit(limit).exec();
  const totalItems = await urldata.countDocuments({ userId: userbyname._id });
  const totalPages = Math.ceil(totalItems / limit);
  res.status(200).json(new ApiResponse(200, { totalItems, page, totalPages, Publicbookmarks }, "User data fetched"));
});
