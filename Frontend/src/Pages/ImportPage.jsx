...
import {
  Upload, FileText, Eye, ArrowRight, CheckCircle2,
  AlertCircle, SkipForward, Check,
  User, Layers, Globe, LayoutDashboard, LogOut, ExternalLink,
} from "lucide-react";
...
export default function ImportPage() {
  ...
  const handlepreview = async () => {
    if (importedfile == null) { setError("Please select a file first"); return; }
    setLoading(true);
    setError("");
    try {
  ...
  const handleconfirm = async () => {
    if (editableLinks.length === 0) { setError("No links to import"); return; }
    setLoading(true);
    setError("");
    try {
      // if user has selected specific rows — import only those, else import all
      const toImport = selectedIndexes.size > 0
        ? editableLinks.filter((_, i) => selectedIndexes.has(i))
        : editableLinks
    ...
    <div style={{ position: "relative", minHeight: "100dvh", display: "flex" }}>
      ...
      >
        ...
        <div style={{ borderTop: "1px solid var(--glass-border)", paddingTop: 12 }}>
          ...
          <button onClick={logout} className="btn-ghost full" style={{ height: 38, fontSize: 13, gap: "8px", justifyContent: "center" }}>
      ...
      <main role="main" style={{ flex: 1, marginLeft: 260, position: "relative", zIndex: 1, padding: "32px 32px 64px" }}>
        ...
        >
          ...
          <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>
            Upload your Chrome bookmarks HTML file and review before importing
          </p>
        </motion.div>

        {/* Step indicator */}
        <motion.div
          {...(reduce ? {} : { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.45, delay: 0.06, ease: [0.16, 1, 0.3, 1] } })}
        ...
        {error && (
          <motion.div
            role="alert" aria-live="polite" className="error-banner"
            {...(reduce ? {} : { initial: { opacity: 0, y: -6 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.25, ease: [0.16, 1, 0.3, 1] } })}
        ...
        {!importResult && (
          <motion.div
            {...(reduce ? {} : { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.5, delay: 0.1, ease: [0.16, 1, 0.3, 1] } })}
          ...
          >
            ...
            <label
              ...
              style={{
                ...
                gap: 12, padding: "36px 24px", borderRadius: "var(--r-lg)",
            ...
            >
              ...
              {importedfile ? (
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-secondary)" }}>
                    {(importedfile.size / 1024).toFixed(1)} KB — click to change
                  </p>
                </div>
              ) : (
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-muted)", maxWidth: "42ch", lineHeight: 1.5 }}>
                    Export from Chrome → Bookmarks Manager → ⋮ menu → Export bookmarks
                  </p>
                </div>
              )}
              ...
              <input
                ...
                style={{ display: "none" }}
                aria-label="Upload Chrome bookmarks HTML file"
              />
            </label>
            ...
            {importedfile && editableLinks.length === 0 && (
              <button
                ...
                style={{ marginTop: 16, height: 48, gap: "8px", color: "#fff" }}
                aria-busy={loading}
              >
                {loading ? (
                  <>
        ...
        {editableLinks.length > 0 && !importResult && (
          <motion.div
            {...(reduce ? {} : { initial: { opacity: 0, y: 16 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.5, delay: 0.05, ease: [0.16, 1, 0.3, 1] } })}
          >
            {/* Section header row */}
            <div className="glass r-lg" style={{ padding: "16px 20px", marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <button
                  ...
                  style={{ height: 42, gap: "8px", padding: "0 20px", color: "#fff" }}
                  aria-busy={loading}
                >
                  {loading ? (
                    <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {editableLinks.map((link, index) => (
                ...
                >
                  ...
                  {/* Favicon avatar */}
                  <div style={{
                    width: 34, height: 34, borderRadius: 8, background: domainColor(link.url),
                  ...
                  {link.confidence && (
                    <span style={{
                      fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 999, whiteSpace: "nowrap", flexShrink: 0,
                  ...
                  {/* Category select */}
                  <GlassSelect
                    value={link.customTag || link.category || "Personal"}
                    ...
                    options={[
                      ...
                      "Knowledge",
                      "Instagram",
                      ...(link.customTag ? [{ value: link.customTag, label: `${link.customTag} (AI)` }] : []),
                    ]}
                    aria-label={`Category for ${link.title || getDomain(link.url)}`}
                    height={32}
                    fontSize={12}
                  />
                  ...
                  <button
                    ...
                    onMouseEnter={e => { e.currentTarget.style.background = "rgb(255 49 98 / 0.12)"; e.currentTarget.style.borderColor = "rgb(255 49 98 / 0.30)"; e.currentTarget.style.color = "var(--accent)"; }}
                    onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "var(--glass-border)"; e.currentTarget.style.color = "var(--text-muted)"; }}
                  >
                    ✕
                  </button>
                </motion.div>
            ...
            <button
              ...
              disabled={loading}
              className="btn-primary full"
              style={{ marginTop: 20, height: 48, gap: "8px", color: "#fff" }}
              aria-busy={loading}
            >
              {loading ? (
                <>
        ...
        {importResult && (
          <motion.div
            {...(reduce ? {} : { initial: { opacity: 0, scale: 0.96, y: 16 }, animate: { opacity: 1, scale: 1, y: 0 }, transition: { duration: 0.5, ease: [0.16, 1, 0.3, 1] } })}
            className="glass-strong r-xl"
            style={{ padding: "clamp(32px,5vw,48px)", textAlign: "center", maxWidth: 520, margin: "0 auto" }}
          >
            <motion.div
              {...(reduce ? {} : { initial: { scale: 0.6, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: { duration: 0.45, delay: 0.1, ease: [0.16, 1, 0.3, 1] } })}
              style={{ marginBottom: 20 }}
            >
              <div style={{
                width: 64, height: 64, borderRadius: "50%",
                background: "rgb(48 209 88 / 0.12)", border: "1px solid rgb(48 209 88 / 0.28)",
                display: "flex", alignItems: "center", justifyContent: "center",
                margin: "0 auto", boxShadow: "0 4px 24px rgb(48 209 88 / 0.18)",
              }} aria-hidden="true">
                <CheckCircle2 size={30} color="#30d158" strokeWidth={1.5} />
              </div>
            </motion.div>

            <h2 style={{ fontSize: "clamp(18px,2.5vw,24px)", fontWeight: 720, letterSpacing: "-0.02em", color: "var(--text-primary)", marginBottom: 8 }}>
              Import Complete
            </h2>
            <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 28 }}>
              Your Chrome bookmarks have been imported successfully.
            </p>

            {/* Stats */}
            <div style={{ display: "flex", gap: 12, justifyContent: "center", marginBottom: 28, flexWrap: "wrap" }}>
              <div className="glass r-lg" style={{ padding: "14px 28px", minWidth: 120 }}>
                <p style={{ fontSize: 32, fontWeight: 760, letterSpacing: "-0.04em", color: "var(--accent)", lineHeight: 1, marginBottom: 4 }}>
                  {importResult.inserted}
                </p>
                <p style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 500 }}>Imported</p>
              </div>
              <div className="glass r-lg" style={{ padding: "14px 28px", minWidth: 120 }}>
                <p style={{ fontSize: 32, fontWeight: 760, letterSpacing: "-0.04em", color: "var(--text-secondary)", lineHeight: 1, marginBottom: 4 }}>
